...
baseSUS <- fread('https://s3-sa-east-1.amazonaws.com/ckan.saude.gov.br/SRAG/2020/INFLUD-05-10-2020.csv',
                 sep2 = ";", header = TRUE)


ocorrenciaBH <- baseSUS %>% 
     filter(CLASSI_FIN == 5 & EVOLUCAO == 2 &
     ID_MUNICIP == "BELO HORIZONTE") %>% 
     select(DT_EVOLUCA) %>% 
     mutate(
...
comparacaoBH <- notificaoesBH %>% 
     full_join(ocorrenciaBH, by = 'data')
